...
public class TileDHD extends BaseTileEntity
{
	...
	void checkForLink()
	{
		System.out.printf("TileDHD.checkForLink at (%d,%d,%d)\n", xCoord, yCoord, zCoord);
		if (!isLinkedToStargate)
		{
			...
			for (int i = -linkRangeX; i <= linkRangeX; i++)
				for (int j = -linkRangeY; j <= linkRangeY; j++)
					...
					{
						Vector3 p = t.p(i, j, -k);
						//System.out.printf("TileDHD: Looking for stargate at (%d,%d,%d)\n",
						//	p.floorX(), p.floorY(), p.floorZ());
						TileEntity te = worldObj.getBlockTileEntity(p.floorX(), p.floorY(), p.floorZ());
						if (te instanceof SGBaseTE)
						{
							//System.out.printf("TileDHD: Found stargate at (%d,%d,%d)\n",
	...
	boolean linkToStargate(SGBaseTE gte)
	{
		...
		{
			System.out.printf(
					"TileDHD: Linking controller at (%d, %d, %d) with stargate at (%d, %d, %d)\n",
	...
	{
		System.out.printf("TileDHD: Unlinking controller at (%d, %d, %d) from stargate\n",
				xCoord, yCoord, zCoord);
